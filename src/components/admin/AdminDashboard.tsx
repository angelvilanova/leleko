...
type Tab = 'home' | 'products' | 'orders' | 'customers' | 'manageOrders' | 'cashier' | 'customerHistory';
  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-slate-950 flex">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Top header */}
        <header className="h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-4 sm:px-6 lg:px-8 sticky top-0 z-20 shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3 min-w-0">
              {currentNav && (
                <>
                  <currentNav.icon className="w-5 h-5 text-blue-600 dark:text-blue-400 shrink-0" />
                  <h2 className="text-lg font-bold text-slate-900 dark:text-white truncate">{currentNav.label}</h2>
                </>
              )}
            </div>
          </div>
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 overflow-y-auto">
          {activeTab === 'home' && <HomeDashboard onNavigate={(tab) => handleNav(tab as Tab)} />}
